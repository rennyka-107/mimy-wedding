...
export default function T2010MyLightTemplate({isPublicPage = false}: {isPublicPage: boolean}) {
    const { template: { configs: { texts, images, background_colors, url_maps, send_gifts } }, setSelectedComponent, updateTemplate, deleteComponent } = useTemplateStore();

    useEffect(() => {
        if (!isPublicPage) updateTemplate(template2010MyLight);
    }, [isPublicPage])


    return (
        <div className="font-afacad relative">
            {!images['image_1'].isDeleted && <img className="w-full" src="/templates/2010MyLight/1.png" alt="first image" />}
            {!images['image_1'].isDeleted && <div
                onClick={(e) => {
                    e.stopPropagation();
            ...
            {!images['image_1'].isDeleted && <svg width="333" height="357" viewBox="0 0 333 357" fill="none" xmlns="http://www.w3.org/2000/svg" className="z-1 absolute top-[55px] left-[48%] -translate-x-1/2">
            ...
            }} style={{ marginTop: images['image_1'].isDeleted ? '1rem' : '13rem', fontWeight: 400, letterSpacing: '0%', lineHeight: '100%', fontFamily: 'Send Flowers', textAlign: 'center', cursor: 'pointer', color: texts['text_1'].text_color, fontSize: texts['text_1'].text_size }}>{texts['text_1'].content}</div>}
